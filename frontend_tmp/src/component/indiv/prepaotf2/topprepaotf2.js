...
axios.get(getAll).then (res => {
  ...
  for (let i = 0; i < games; i++) {
    // KDA
    let kills = parseInt(((res.data[i]).KDA1).split("/")[0])
    let deaths = parseInt(((res.data[i]).KDA1).split("/")[1])
    let assists = parseInt(((res.data[i]).KDA1).split("/")[2])
    ...
    // cs
     let cs = ((res.data[i]).CS1)
     let minutes = parseInt(((res.data[i]).Duration).split(":")[0])
    csMin += (cs/minutes)
    // dégats
    let degatTop = ((res.data[i]).Degat1)
    ...
    // Gold Diff @ 8
    let goldDiff_8 = parseInt(((res.data[i]).GoldDiff1_8))
    GoldDiff_8Tot += goldDiff_8
    // Gold Diff @ 14
    let goldDiff_14 = parseInt(((res.data[i]).GoldDiff1_14))
    GoldDiff_14Tot += goldDiff_14
    // Gold Diff @ 22
    let goldDiff_22 = parseInt(((res.data[i]).GoldDiff1_22))
    GoldDiff_22Tot += goldDiff_22
    // Gold Diff Tot
    let goldDiff = parseInt(((res.data[i]).GoldDiff1))
    GoldDiffTot += goldDiff

    // Pinks
    let pinks = ((res.data[i]).Pink1)
...
export default StatsPrepaotf2Top